...
let ResultBox = document.querySelector(".insight");
...
async function generateInsights() {
    try {
        ...
        ResultBox.textContent = "Error generating insights. Please try again.";
    }    
}

function displayInsights(insightText) {
    const words = insightText.split(" ");
    ResultBox.textContent = "";

    words.forEach((word, index) => {
        setTimeout(() => {
            ResultBox.textContent += (index === 0 ? "" : " ") + word;
        }, index * 100);
    });
}
